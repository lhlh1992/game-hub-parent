# Keycloak + Gateway 部署指南

> 📅 **创建日期**: 2025-01-XX  
> 📝 **适用范围**: game-hub-parent 微服务架构

---

## 📋 目录

1. [部署概览](#1-部署概览)
2. [前置准备](#2-前置准备)
3. [部署步骤](#3-部署步骤)
4. [Keycloak 配置](#4-keycloak-配置)
5. [验证部署](#5-验证部署)
6. [常见问题](#6-常见问题)

---

## 1. 部署概览

### 1.1 需要部署的服务

本次部署包含以下服务（通过 Docker Compose 统一管理）：

| 服务 | 说明 | 端口 | 状态 |
|------|------|------|------|
| **PostgreSQL** | 数据库（Keycloak 数据存储） | 5432 | ✅ 已有 |
| **Redis** | 缓存服务 | 6379 | ✅ 已有 |
| **Keycloak** | OAuth2/OIDC 认证服务器 | 8180 | 🆕 新增 |
| **Gateway** | Spring Cloud Gateway 网关 | 8080 | 🆕 新增 |

### 1.2 服务依赖关系

```
Gateway → Keycloak (JWT验证)
Gateway → Redis (限流/缓存)
Keycloak → PostgreSQL (数据存储)
```

### 1.3 网络架构

所有服务运行在 Docker 网络 `pgnet` 中（172.30.0.0/24），使用固定 IP：

- PostgreSQL: `172.30.0.10`
- pgAdmin: `172.30.0.11`
- Redis: `172.30.0.12`
- **Keycloak**: `172.30.0.13` 🆕
- **Gateway**: `172.30.0.14` 🆕

---

## 2. 前置准备

### 2.1 环境要求

- ✅ Docker Desktop (Windows/Mac) 或 Docker Engine (Linux)
- ✅ Docker Compose v2.0+
- ✅ 已启动 PostgreSQL 和 Redis 服务

### 2.2 检查现有服务

```bash
# 进入项目目录
cd d:\workspace\java\game-hub-parent

# 检查现有服务状态
docker-compose ps

# 确保 PostgreSQL 和 Redis 正在运行
# 如果没有运行，先启动：
docker-compose up -d postgres redis
```

### 2.3 创建 Keycloak 数据库

Keycloak 需要使用 PostgreSQL 中的 `keycloak` 数据库，需要手动创建：

**方式1：使用 pgAdmin（推荐）**

1. 访问 http://localhost:5050 登录 pgAdmin
2. 连接到 PostgreSQL 服务器
3. 右键点击 `Databases` → `Create` → `Database`
4. 填写：
   - **Database name**: `keycloak`
   - **Owner**: `postgres`
5. 点击 `Save`

**方式2：使用 psql 命令行**

```bash
# 进入 PostgreSQL 容器
docker exec -it pgsql psql -U postgres

# 创建数据库
CREATE DATABASE keycloak;

# 退出
\q
```

**方式3：使用 SQL 脚本**

在 `D:/DockerData/pgsql/init/` 目录下创建文件 `01-create-keycloak-db.sql`：

```sql
CREATE DATABASE keycloak;
```

重启 PostgreSQL 服务即可自动执行。

---

## 3. 部署步骤

### 3.1 构建 Gateway 镜像

```bash
# 在 game-hub-parent 目录下执行
docker-compose build gateway
```

**预期输出：**
- 下载 Maven 依赖
- 编译 Gateway 代码
- 构建 Docker 镜像

### 3.2 启动 Keycloak

```bash
# 启动 Keycloak 服务
docker-compose up -d keycloak

# 查看启动日志
docker-compose logs -f keycloak
```

**等待 Keycloak 启动完成**（约 1-2 分钟），直到看到：
```
Keycloak 25.0.1 started in XXms
```

### 3.3 启动 Gateway

```bash
# 启动 Gateway 服务（会自动等待 Keycloak 和 Redis 就绪）
docker-compose up -d gateway

# 查看启动日志
docker-compose logs -f gateway
```

### 3.4 验证服务状态

```bash
# 查看所有服务状态
docker-compose ps

# 应该看到所有服务都是 "Up" 状态
```

**预期输出：**
```
NAME                  STATUS
gamehub-gateway       Up (healthy)
gamehub-keycloak      Up (healthy)
pgsql                 Up
redis                 Up (healthy)
pgadmin               Up
```

---

## 4. Keycloak 配置

### 4.1 访问 Keycloak Admin Console

1. 打开浏览器访问：http://localhost:8180
2. 点击右上角 `Administration Console`
3. 登录：
   - **Username**: `admin`
   - **Password**: `admin`

### 4.2 创建 Realm

1. 鼠标悬停在左上角 `Master` Realm
2. 点击 `Create Realm`
3. 填写：
   - **Realm name**: `gamehub`
4. 点击 `Create`

### 4.3 创建 Client（客户端）

1. 在左侧菜单选择 `Clients`
2. 点击 `Create client`
3. **General settings** 页面：
   - **Client type**: `OpenID Connect`
   - **Client ID**: `game-platform`
4. 点击 `Next`
5. **Capability config** 页面：
   - ✅ `Client authentication`: `Off`（公共客户端）
   - ✅ `Standard flow`: `On`（授权码流程）
   - ✅ `Direct access grants`: `On`（密码模式）
6. 点击 `Next`
7. **Login settings** 页面：
   - **Valid redirect URIs**: `http://localhost:3000/*`（前端地址，按需调整）
   - **Web origins**: `*`（开发环境，生产应限制）
   - **Post logout redirect URIs**: `http://localhost:3000/*`
8. 点击 `Save`

### 4.4 创建 Roles（角色）

1. 在左侧菜单选择 `Realm roles`
2. 点击 `Create role`
3. 依次创建以下角色：
   - **Role name**: `PLAYER` → `Save`
   - **Role name**: `GUEST` → `Save`
   - **Role name**: `ADMIN` → `Save`

### 4.5 创建测试用户

#### 5.1 创建用户

1. 在左侧菜单选择 `Users`
2. 点击 `Create new user`
3. 填写：
   - **Username**: `testuser`
   - **Email**: `test@example.com`（可选）
   - ✅ **Email verified**: `On`
4. 点击 `Create`

#### 5.2 设置密码

1. 点击 `Credentials` 标签
2. 设置密码（如 `password123`）
3. ⚠️ **关闭** `Temporary`（避免首次登录要求修改密码）
4. 点击 `Set password`，确认

#### 5.3 分配角色

1. 点击 `Role mapping` 标签
2. 点击 `Assign role`
3. 选择 `Filter by realm roles`
4. 勾选 `PLAYER`
5. 点击 `Assign`

---

## 5. 验证部署

### 5.1 验证 Keycloak

```bash
# 健康检查
curl http://localhost:8180/health/ready

# 获取 Token（密码模式）
curl -X POST http://localhost:8180/auth/realms/gamehub/protocol/openid-connect/token \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=password" \
  -d "client_id=game-platform" \
  -d "username=testuser" \
  -d "password=password123"
```

**预期响应：**
```json
{
  "access_token": "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9...",
  "expires_in": 300,
  "refresh_token": "...",
  "token_type": "Bearer",
  "scope": "profile email"
}
```

### 5.2 验证 Gateway

```bash
# 健康检查
curl http://localhost:8080/actuator/health

# 使用 Token 访问受保护接口
curl -X GET http://localhost:8080/api/game/test \
  -H "Authorization: Bearer <access_token>"
```

**预期结果：**
- Gateway 验证 JWT Token
- 提取用户信息并注入到请求头：`X-User-Id`, `X-Username`, `X-Roles`
- 转发请求到下游服务

### 5.3 查看 Gateway 日志

```bash
# 查看用户信息注入日志
docker-compose logs gateway | grep "JWT 用户信息"
```

**预期输出：**
```
JWT 用户信息 - userId: f:xxx:testuser, username: testuser, roles: PLAYER
```

---

## 6. 常见问题

### Q1: Keycloak 启动失败，提示数据库连接错误？

**A:** 检查：
1. ✅ PostgreSQL 是否正在运行
2. ✅ `keycloak` 数据库是否已创建
3. ✅ 数据库连接配置是否正确（用户名、密码、IP地址）

```bash
# 检查数据库是否存在
docker exec -it pgsql psql -U postgres -l | grep keycloak
```

### Q2: Gateway 无法连接到 Keycloak？

**A:** 检查：
1. ✅ Keycloak 是否已启动并健康
   ```bash
   docker-compose ps keycloak
   curl http://localhost:8180/health/ready
   ```
2. ✅ Gateway 配置中的 `KEYCLOAK_ISSUER_URI` 是否正确
3. ✅ 网络连接是否正常（同一 Docker 网络）
4. ✅ 查看 Gateway 日志：
   ```bash
   docker-compose logs gateway | grep -i "keycloak\|jwt\|token"
   ```

### Q3: Gateway 验证 JWT 失败？

**A:** 检查：
1. ✅ Realm 名称是否一致（`gamehub`）
2. ✅ Token 是否过期（默认 5 分钟）
3. ✅ Token 格式是否正确（`Bearer <token>`）
4. ✅ Keycloak 的 JWK Set 端点是否可访问：
   ```bash
   curl http://localhost:8180/auth/realms/gamehub/protocol/openid-connect/certs
   ```

### Q4: Gateway 构建失败？

**A:** 检查：
1. ✅ Java 代码是否有编译错误
2. ✅ Maven 依赖是否正常下载
3. ✅ 查看构建日志：
   ```bash
   docker-compose build --no-cache gateway
   ```

### Q5: 如何重新构建 Gateway？

```bash
# 重新构建并启动
docker-compose up -d --build gateway

# 强制重新构建（不使用缓存）
docker-compose build --no-cache gateway
docker-compose up -d gateway
```

### Q6: 如何重置 Keycloak 配置？

```bash
# 停止服务
docker-compose stop keycloak

# 删除 Keycloak 数据卷（⚠️ 会删除所有配置）
docker-compose down -v keycloak

# 重新创建数据库
# （参考 2.3 节）

# 重新启动
docker-compose up -d keycloak
```

---

## 7. 服务管理命令

### 启动服务

```bash
# 启动所有服务
docker-compose up -d

# 启动指定服务
docker-compose up -d keycloak gateway
```

### 停止服务

```bash
# 停止所有服务
docker-compose stop

# 停止指定服务
docker-compose stop gateway
```

### 查看日志

```bash
# 查看所有服务日志
docker-compose logs -f

# 查看指定服务日志
docker-compose logs -f gateway
docker-compose logs -f keycloak
```

### 重启服务

```bash
# 重启所有服务
docker-compose restart

# 重启指定服务
docker-compose restart gateway
```

### 删除服务

```bash
# 停止并删除容器（数据卷保留）
docker-compose down

# 停止并删除容器和数据卷（⚠️ 会删除所有数据）
docker-compose down -v
```

---

## 8. 生产环境注意事项

1. **修改默认密码**：更改 Keycloak Admin 密码
2. **使用 HTTPS**：配置 SSL 证书
3. **限制 Web origins**：不要使用 `*`，指定具体域名
4. **配置数据库持久化**：确保 PostgreSQL 数据卷持久化
5. **配置备份策略**：定期备份 Keycloak 数据
6. **监控与日志**：接入 Prometheus、Grafana 等监控系统
7. **资源限制**：在 docker-compose.yml 中为各服务配置资源限制
8. **密钥管理**：使用 Docker Secrets 或环境变量管理敏感信息

---

## 9. 参考资源

- [Keycloak 官方文档](https://www.keycloak.org/documentation)
- [Spring Cloud Gateway 文档](https://spring.io/projects/spring-cloud-gateway)
- [Docker Compose 文档](https://docs.docker.com/compose/)

---

> 💡 **提示**: 如遇到问题，请先查看服务日志，大多数问题都可以从日志中找到原因。




