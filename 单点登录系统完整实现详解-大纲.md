...
> **本章目标**：理解业务需求和技术挑战，明确为什么需要这个方案，以及为什么选择 `sid` 作为 `loginSessionId`。
...
**业务场景**：
- 用户通过 WebSocket 连接进行实时通信（如游戏、聊天）
...
**下一步**：理解了问题和需求后，我们可以继续学习核心设计思想，了解如何解决这些问题。
...
**所有服务统一走"三层校验"**：
1. **Token 签名校验**：防止伪造
2. **黑名单校验**：立即失效已撤销的 token
3. **会话状态校验**：判断 loginSession 是否被踢
...
**下一步**：理解了设计思想后，我们可以继续学习具体的实现细节，了解如何将这些设计思想转化为代码。
...
- **业务策略**：同一时刻只允许一个登录会话处于 `ACTIVE` 状态
...
- **包含信息**：用户 ID（sub）、角色（roles）、会话 ID（sid）等
...
- ✅ 会话状态（ACTIVE/KICKED/EXPIRED）用于控制访问权限
- ✅ 所有资源（Token、WebSocket）都通过 `loginSessionId` 关联

**下一步**：理解了这些概念后，我们可以继续学习背景与问题分析，了解为什么需要这个方案。
...
3. 用户在 Keycloak 登录
4. Keycloak 回调 Gateway，携带授权码
5. Gateway 使用授权码换取 access_token
6. Spring Security 创建 Authentication
...
3. ✅ **标记为 KICKED**：旧会话标记为 KICKED，不删除
...
**下一步**：理解了登录流程后，我们可以继续学习 JWT 校验流程，了解如何验证 token 的有效性。
...
**步骤7：验证 token 的 jti 与会话的 sessionId 匹配**
```java
String sessionJti = sessionInfo.getSessionId();
if (!jwtJti.equals(sessionJti)) {
    // Token 的 jti 与会话的 sessionId 不匹配，说明 token 已被覆盖
    return Mono.just(ResponseEntity.status(401).body(
            Map.<String, Object>of("error", "Token 已失效", "message", "请重新登录")
    ));
}
...
**关键点**：
- ✅ **防止 token 被覆盖**：如果 token 的 `jti` 与会话的 `sessionId` 不匹配，说明 token 已被其他登录覆盖
- ✅ **精确匹配**：确保返回的 token 属于当前会话
...
**验证3：token 的 jti 与会话的 sessionId 匹配**
```java
if (!jwtJti.equals(sessionJti)) {
    // Token 的 jti 与会话的 sessionId 不匹配，说明 token 已被覆盖
    return 401;
...
**下一步**：理解了 Token 获取接口后，我们可以继续学习 SessionRegistry 核心实现，了解会话管理的底层机制。
...
5. 移除 `OAuth2AuthorizedClient`
6. 调用 Keycloak OIDC 登出接口
...
调用 Keycloak OIDC 登出接口
  ↓
重定向到首页（/game-service/lobby.html）
  ↓
登出完成
```
...
**不清理项**：
- ❌ **SessionRegistry 中的会话记录**：保留审计记录（可选：标记为 EXPIRED）
...
**下一步**：理解了登出流程后，我们已经完成了核心流程的学习。接下来可以学习核心组件的实现细节。
...
2. 发送 STOMP CONNECT 帧（携带 JWT token）
3. 服务器验证 token，建立连接
4. 客户端订阅目标地址
5. 服务器推送消息
...
客户端建立 WebSocket 连接
  ↓
ws://localhost:8080/ws?access_token=xxx
  ↓
...
- `JwtGrantedAuthoritiesConverter`：权限转换器，从 JWT 提取权限
...
**下一步**：理解了 WebSocket 连接管理后，我们可以继续学习 Kafka 事件通知机制，了解如何通过事件通知各服务断开 WebSocket 连接。
...
**问题**：
- Gateway 服务处理登录/登出
- Game-Service 服务管理 WebSocket 连接
- 需要跨服务通知：登录/登出时，断开 WebSocket 连接

**解决方案**：
- **Kafka 事件通知**：Gateway 发布事件，Game-Service 消费事件
- **解耦**：Gateway 不需要直接调用 Game-Service
- **扩展性**：多个服务可以订阅同一事件
...
**文件位置**：`libs/session-kafka-notifier/src/main/java/com/gamehub/sessionkafkanotifier/event/SessionInvalidatedEvent.java`
...
**下一步**：理解了 Kafka 事件通知机制后，我们已经完成了扩展功能的学习。接下来可以学习总结与深化部分。
...
1. ✅ **查询 ACTIVE 会话**：只查询该 `userId` 的 ACTIVE 会话
2. ✅ **跳过同一 loginSessionId**：token 刷新场景，不踢掉自己
3. ✅ **标记为 KICKED**：旧会话标记为 KICKED，不删除（保留审计记录）
4. ✅ **注册新会话**：新会话状态为 ACTIVE
...
**下一步**：理解了 SessionRegistry 后，我们可以继续学习 JWT 黑名单机制，了解如何立即失效已撤销的 token。
...
**下一步**：理解了 JWT 黑名单机制后，我们已经完成了核心组件的学习。接下来可以学习扩展功能（WebSocket 连接管理、Kafka 事件通知机制）。
...
5. **业务逻辑**：通过 `sha.getUser()` 获取用户身份

**关键点**：
- ✅ Token 只在 STOMP CONNECT 时使用一次
- ✅ 用户身份存储在 WebSocket 会话中
- ✅ 后续消息不再需要携带 token
- ✅ 业务逻辑从消息 header 中获取用户身份（Spring 框架已从会话中设置）
...
**下一步**：理解了完整数据流后，我们可以继续学习关键设计决策，了解为什么做出这些设计选择。
...
- ✅ **保留记录**：可以查看被踢下线的会话信息
- ✅ **时间戳**：可以查看被踢下线的时间
- ✅ **设备信息**：可以查看被踢下线的设备信息（IP、User-Agent）

**示例**：
```
用户 A 的登录历史：
  - 2024-01-01 10:00:00 登录（设备1，loginSessionId="sid-001", status=KICKED）
  - 2024-01-01 10:05:00 登录（设备2，loginSessionId="sid-002", status=ACTIVE）
...
**下一步**：理解了关键设计决策后，我们可以继续学习边界场景处理，了解如何处理各种边界情况。
...
**下一步**：理解了边界场景处理后，我们已经完成了总结与深化的学习。接下来可以学习测试验证部分。
...
**下一步**：理解了测试验证方法后，我们可以继续学习常见问题与解决方案，了解如何排查和解决实际问题。
...
**下一步**：理解了性能优化建议后，我们可以继续学习监控与运维，了解如何监控系统运行状态。
