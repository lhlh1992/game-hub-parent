...
3. **TICK**：每秒广播剩余时间到房间（`TICK`事件，包含left、side、deadlineEpochMs）
...
#### 10.1.1 代码组织
- **GomokuServiceImpl过大**（1175行）：建议拆分
...
- **内存与Redis双重存储**：可能导致不一致
  ...
  - 建议：统一以Redis为准，内存仅作缓存，定期同步或使用事件驱动更新
...
#### 10.1.4 代码问题
- **useGomokuGame Hook过大**（748行）：建议拆分
  - 状态管理Hook
  - WebSocket管理Hook
  - 操作方法Hook
- **GameRoomPage过大**：建议拆分组件
  - 棋盘组件
  - 玩家面板组件
  - 聊天组件
