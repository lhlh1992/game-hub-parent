...
| 社交域 | `chat-service` | 大厅/房间聊天、私聊、频道、消息持久化、禁言、系统通知推送 | WebSocket, Kafka, Redis, PostgreSQL |
...
- ✅ **system-service**：用户管理（CRUD）、PostgreSQL 持久化、Keycloak 事件监听、会话监控 API、用户同步、好友系统、通知系统、文件存储
- ✅ **game-service**：五子棋完整功能（PVP/PVE）、WebSocket 实时对局、倒计时系统、AI 引擎、房间管理、CAS 并发控制
- ✅ **chat-service**：大厅/房间聊天、私聊消息、系统通知推送、消息持久化（Redis + PostgreSQL）、WebSocket 实时通信
...
**服务**：
- ✅ **chat-service**：聊天服务已实现，支持大厅/房间聊天、私聊、系统通知推送，前端已接入
...
**PostgreSQL**（已使用）：
- system-service：用户表（sys_user）、用户扩展表（sys_user_profile）、角色表（sys_role）、权限表（sys_permission）、好友表（friend_request、user_friend）、通知表（sys_notification）等
- chat-service：聊天消息表（chat_message）、聊天会话表（chat_session）、会话成员表（chat_session_member）
...
**已使用（chat-service）**：
- ✅ PostgreSQL：聊天消息历史（`chat_message`、`chat_session`、`chat_session_member` 表）

**已使用（system-service）**：
- ✅ MinIO：头像、游戏回放、活动素材文件存储（已实现）

**未使用**：
- ❌ MongoDB：聊天历史（规划中，当前使用 PostgreSQL）
...
本项目采用**微服务架构**而非单体架构，主要基于以下考虑：

### 4.5.1 业务特性决定
- **多游戏类型**：平台规划支持五子棋、象棋、卡牌等多种游戏，每种游戏的规则、AI、状态管理差异较大，需要独立演进。
- **不同扩展需求**：游戏服务（高并发、实时性）与用户服务（数据持久化、权限管理）的扩展策略不同，单体难以兼顾。
- **独立部署需求**：游戏逻辑更新不应影响用户管理、认证等核心服务，微服务支持独立部署和回滚。

### 4.5.2 技术特性决定
- **技术栈差异**：游戏服务需要 WebSocket 实时通信、Redis 状态管理；用户服务需要 PostgreSQL 持久化、Keycloak 集成。单体架构难以统一技术选型。
- **资源隔离**：游戏对局的 CPU/内存消耗与用户查询的数据库连接消耗需要隔离，避免相互影响。
- **故障隔离**：游戏服务异常不应影响用户登录、权限管理等核心功能，微服务提供天然故障边界。

### 4.5.3 团队协作决定
- **并行开发**：不同团队可以并行开发游戏服务、用户服务、聊天服务，减少代码冲突和依赖。
- **技术债务隔离**：游戏逻辑的快速迭代不会污染用户管理等稳定模块的代码质量。

### 4.5.4 权衡说明
- **复杂度增加**：服务间通信、分布式事务、服务发现等带来额外复杂度，但通过 Gateway 统一入口、Kafka 事件解耦、Redis 共享状态等方式控制复杂度。
- **运维成本**：需要 Docker Compose/K8s 部署、服务监控、日志聚合等，但这是云原生架构的标配能力。

**结论**：对于多游戏平台这类业务边界清晰、扩展需求差异大的场景，微服务架构是合理选择。单体架构更适合业务简单、团队小、快速迭代的早期项目。
...
│   ├── system-service/    ← 用户服务（用户管理、权限、PostgreSQL 持久化、好友系统、通知系统、文件存储）
│   ├── chat-service/      ← 聊天服务（大厅/房间聊天、私聊、系统通知推送、消息持久化）
│   └── pom.xml
├── libs/                  ← 共享库
...
**说明**：
- **后端**：四个核心服务（gateway、game-service、system-service、chat-service）+ 三个共享库（session-common、session-kafka-notifier、web-common）
- **前端**：独立的 React SPA 应用（game-hub-web），通过 Gateway 访问后端 API，已接入 chat-service
...
### 5.5 `apps/chat-service` 类职责

| 类 / 包 | 功能定位 |
|---------|----------|
| `ChatServiceApplication` | 聊天服务启动入口。 |
| `config.SecurityConfig` | Spring Security / OAuth2 资源服务配置。 |
| `config.WebSocketStompConfig` | WebSocket STOMP 配置（消息代理、端点注册）。 |
| `controller.http.ChatRestController` | HTTP REST 接口（发送消息、查询历史等）。 |
| `controller.http.ChatHistoryController` | 聊天历史查询接口（大厅、房间、私聊历史）。 |
| `controller.http.ChatSessionController` | 会话管理接口（创建/查询会话、标记已读等）。 |
| `controller.http.NotificationInternalController` | 内部通知推送接口（供 system-service 调用）。 |
| `controller.ws.ChatWsController` | WebSocket STOMP 控制器（消息收发、订阅管理）。 |
| `entity.ChatMessage` | 聊天消息实体（PostgreSQL 持久化）。 |
| `entity.ChatSession` | 聊天会话实体（大厅、房间、私聊会话）。 |
| `entity.ChatSessionMember` | 会话成员实体（会话与用户的关联）。 |
| `infrastructure.client.SystemUserClient` | System 服务客户端（Feign Client，调用 system-service 获取用户信息）。 |
| `repository.ChatMessageRepository` | 消息仓储接口。 |
| `repository.ChatSessionRepository` | 会话仓储接口。 |
| `repository.ChatSessionMemberRepository` | 会话成员仓储接口。 |
| `service.ChatMessagingService` | 消息收发服务接口。 |
| `service.impl.ChatMessagingServiceImpl` | 消息收发服务实现（消息验证、广播、持久化）。 |
| `service.ChatSessionService` | 会话管理服务接口。 |
| `service.impl.ChatSessionServiceImpl` | 会话管理服务实现（创建/查询会话、成员管理）。 |
| `service.ChatHistoryService` | 聊天历史服务接口。 |
| `service.impl.ChatHistoryServiceImpl` | 聊天历史服务实现（Redis 缓存 + PostgreSQL 持久化）。 |
| `service.NotificationPushService` | 系统通知推送服务接口。 |
| `service.impl.NotificationPushService` | 系统通知推送服务实现（通过 WebSocket 推送通知到前端）。 |
| `service.UserProfileCacheService` | 用户信息缓存服务（Redis 缓存，跨服务共享）。 |
| `ws.WebSocketAuthChannelInterceptor` | WebSocket 鉴权拦截器（验证 JWT Token，注入用户信息）。 |
| `ws.WebSocketSessionManager` | WebSocket Session 管理器（用户与 Session 映射）。 |
| `ws.WebSocketDisconnectHelper` | WebSocket 断开连接处理（清理订阅、通知其他用户）。 |
| `ws.SessionInvalidatedListener` | 会话失效监听器（监听登录/登出事件，清理 WebSocket 连接）。 |
...
- **服务间同步通信**
  ...
  - **调用方式**：
    ...
    - `system-service` → `chat-service`：通过 Feign Client 调用 `/api/internal/notify` 接口推送系统通知。
    - 调用地址通过配置项注入，例如 `system-service.url`、`chat-service.url`，支持本地（`http://localhost:8082`、`http://localhost:8083`）、Docker Compose（`http://system-service:8082`、`http://chat-service:8083`）、K8s（`http://system-service`、`http://chat-service`）三套环境切换。
...
- **对象存储**
  ...
  - **当前状态**：✅ 已实现，system-service 已集成 MinIO，支持头像上传、游戏回放上传、活动素材上传。
...
2. **聊天室 MVP（P0）**：
   ...
   - **当前状态**：✅ 已实现，chat-service 支持大厅/房间聊天、私聊、系统通知推送，前端已接入
...
5. **数据持久化（P1）**：
   ...
   - **当前状态**：✅ system-service 用户数据已持久化到 PostgreSQL，✅ chat-service 聊天历史已持久化到 PostgreSQL，✅ system-service 已实现 MinIO 文件存储（头像、游戏回放、活动素材），❌ 对局记录未持久化
...
- **聊天室 MVP**：✅ 已实现
  - **当前状态**：chat-service 已实现，支持大厅/房间聊天、私聊、系统通知推送，前端已接入
  - **已实现功能**：公共大厅聊天、房间聊天、私聊消息、消息持久化（Redis + PostgreSQL）、系统通知推送
  - **下一步**：完善禁言/屏蔽功能、敏感词过滤、发送频率限制
- **玩家中心雏形**：❌ 未实现
