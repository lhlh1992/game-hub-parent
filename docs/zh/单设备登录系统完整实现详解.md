...
> 教学级文档：涵盖登录、登出、JWT黑名单、WebSocket管理、单设备登录（后连踢前）、Kafka通知WS断连等完整实现
...
本认证系统采用 **Keycloak + Gateway + Kafka + Redis** 的微服务架构，实现单设备登录、会话管理和 WebSocket 连接控制。
...
- **OAuth2 角色**：Resource Server（资源服务器）
- **职责**：处理 WebSocket 连接管理（作为单设备登录系统的下游服务示例）
- **核心功能**：
...
**关联的作用**：
- ✅ **单设备登录**：新登录时，可以基于 `loginSessionId` 查询并断开旧连接的 WebSocket
...
> **本章目标**：理解完整的登录流程，掌握从用户登录到会话注册、单设备登录处理的每个步骤和代码实现细节。
...
3. 调用 `SessionRegistry.registerLoginSessionEnforceSingle()`（单设备登录核心）
...
- ✅ **同步注销 Keycloak 会话**：调用 `keycloakSsoLogoutService.logout()` 注销 Keycloak 端的会话
- ✅ **错误处理**：记录错误日志，不抛出异常

**完整实现**（包含 Keycloak 会话注销）：
```java
for (LoginSessionInfo kickedSession : kickedSessions) {
    String kickedLoginSessionId = kickedSession.getLoginSessionId();
    if (kickedLoginSessionId == null || kickedLoginSessionId.isBlank()) {
...
- 新的 `KeycloakSsoLogoutService.logout(userId, loginSessionId)` 会优先调用 Keycloak Admin API 的 `deleteSession(loginSessionId, false)` 精确注销对应的在线会话，会话 ID 缺失时再回退到 `users().get(userId).logout()`（全量清掉该用户的会话）。
- 触发位置：
  ...
  3. `LoginSessionKickHandler.publishKickedEvent()`：单设备登录“后连踢前”。
...
3. 单设备登录注册（`registerLoginSessionEnforceSingle`）
...
- `extractSid()`：sid 提取
- `registerLoginSessionEnforceSingle()`：单设备登录核心
...
- ✅ **单设备登录问题**：如果 token 已被其他登录覆盖，拒绝返回 token
...
5. 用户在Keycloak登录
6. Keycloak回调Gateway，Gateway处理登录（单设备登录、黑名单、事件发布）
7. Gateway重定向回前端
...
**后端**（Gateway）：
- 处理OAuth2授权码流程
- 调用 `LoginSessionKickHandler` 实现单设备登录
...
- `WebSocketSessionManager`：注册WebSocket会话，实现单设备登录（踢旧连接）
...
> **本章目标**：理解 WebSocket 连接管理的完整实现，掌握如何从 JWT 中提取 `loginSessionId`、如何实现 WebSocket 单设备登录、如何踢掉旧连接。
...
5. **后端注册会话**：`WebSocketSessionManager` 注册WebSocket会话到SessionRegistry，实现单设备登录（踢旧连接）
...
public void handleSessionConnect(SessionConnectEvent event) {
    ...
    if (!CollectionUtils.isEmpty(kicked)) {
        log.info("用户 {} WebSocket 单设备登录，新连接 {} 踢掉旧连接 {} 个, loginSessionId={}", 
...
4. ✅ **单设备登录注册**：调用 `registerWebSocketSessionEnforceSingle()`，返回被踢掉的旧连接
5. ✅ **处理旧连接**：发送踢人通知并强制断开
...
- `registerLoginSessionEnforceSingle()`：单设备登录注册
...
> **本章目标**：通过完整的数据流图，理解整个单设备登录系统的数据流转过程，包括登录、JWT校验、WebSocket连接、登出等完整流程。
...
- 提取 JWT 信息
- 构建 `LoginSessionInfo`
- 调用 `registerLoginSessionEnforceSingle()` 实现单设备登录
...
**核心数据流**：
1. **登录数据流**：用户登录 → 会话注册 → 单设备登录处理 → 黑名单 → 事件发布
2. **JWT校验数据流**：三层校验（黑名单 → 签名 → 状态）
3. **WebSocket连接数据流**：连接建立 → 会话注册 → 单设备登录检查 → 踢掉旧连接
...
> **本章目标**：掌握如何测试单设备登录系统的各项功能，包括功能测试、边界场景测试、验证方法等。
...
**测试场景**：
1. **功能测试**：单设备登录、Token刷新、WebSocket断连、登出
...
**SessionRegistry**：
- `registerLoginSessionEnforceSingle()`：注册登录会话（单设备登录）
- `getLoginSessionByLoginSessionId()`：按 loginSessionId 查询
- `updateSessionStatus()`：更新会话状态
- `registerWebSocketSessionEnforceSingle()`：注册 WebSocket 会话（单设备登录）
...
**文档完成**：至此，我们已经完成了单设备登录系统完整实现详解文档的所有内容。希望这份文档能够帮助你深入理解整个系统的实现原理和细节。
