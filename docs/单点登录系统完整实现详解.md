...
│  Client (客户端): 浏览器/前端 (Browser/Frontend), Gateway (OAuth2 Client)      │
...
└─────────────────────────────────────────────────────────────────────────────┘
        ...
        │   Redis (SSO 状态存储)                                    │
        │                                                          │
        │  • session:login:{loginSessionId} (LoginSessionInfo)     │
...
├───────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│  1. 登录流程:                                                                								  │
│     User → Browser → Gateway (OAuth2 Client) → Keycloak → Gateway → User   								│
│                                                                              								│
│  2. 登出流程:                                                                								  │
│     User → Gateway → Redis (token黑名单) →  keycloak注销旧会话  → Kafka消息 → 应用服务 (断开 WebSocket)		   │
│                                                                              								 │
│  3. 单点登录 (后连踢前):                                                     									│
│     新登录 → Gateway → Redis (标记旧会话 KICKED) →  keycloak注销旧会话 →  Kafka消息 → 应用服务(断开 WebSocket)     │
│                                                                              								 │
│  4. 资源访问:                                                                								   │
...
**2. Browser / Frontend（浏览器/前端 / Client）**
- **OAuth2 角色**：Client（客户端）
- **职责**：作为用户代理，发起 OAuth2 授权请求，携带 JWT Token 访问资源
- **关键功能**：
  - 访问 Gateway 受保护资源
...
**4. Gateway（网关服务 / OAuth2 Client + Resource Server）**
- **OAuth2 角色**：
  - **OAuth2 Client**：与 Keycloak 进行 OAuth2 授权码流程
...
- **核心功能**：
  ...
  - **Websocket加请求头**：过滤器将ws的请求头加上token
- **关键设计**：
...
Gateway API
    │
    │ 4. 解析 JWT，提取 sid
    │ 5. 存储到 Redis: session:login:{sid}
    │ 6. 注册到 SessionRegistry
    │ 7. 检查是否有旧会话，如有则踢掉（发布 Kafka 事件）
    ▼
用户浏览器（获得 Session Cookie）
```
...
Gateway API
    │
    │ 1. 从 Session 获取 OAuth2AuthorizedClient
    │ 2. 检查 SessionRegistry 中的会话状态
    │ 3. 如果状态为 KICKED，返回 401
    │ 4. 如果状态为 ACTIVE，返回 access_token
    ▼
用户浏览器（获得 JWT Token）
```
...
Gateway API
    │
    │ 1. 提取当前 loginSessionId（sid）
    │ 2. 将 JWT 写入黑名单
    │ 3. 更新 SessionRegistry：状态改为 KICKED
    │ 4. 发布 Kafka 事件：SESSION_INVALIDATED
    │ 5. 清除 Gateway Session
    │ 6. 调用 Keycloak 登出接口
    ▼
Kafka
    │
    │ 事件：SESSION_INVALIDATED
    ▼
game-service
    │
    │ 1. 监听 Kafka 事件
    │ 2. 查找 Redis: ws:session:{loginSessionId}
    │ 3. 断开对应的 WebSocket 连接
    ▼
用户浏览器（WebSocket 连接断开）
```

#### 4. 后连踢前流程

```
设备 1：已登录，WebSocket 连接活跃
    │
设备 2：用户登录
    │
    ▼
Gateway API
    │
    │ 1. 检测到设备 1 的旧会话
    │ 2. 更新设备 1 的会话状态为 KICKED
    │ 3. 发布 Kafka 事件：SESSION_INVALIDATED（loginSessionId = 设备1的sid）
    │ 4. 注册设备 2 的新会话
    ▼
Kafka
    │
