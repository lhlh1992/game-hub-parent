...
Gateway API
    ...
    │ 6. 调用 KeycloakSsoLogoutService（Keycloak Admin API）失效对应的 SSO 会话
    ▼
Kafka
    │
...
Gateway API
    │
    │ 1. 检测到设备 1 的旧会话
    │ 2. 在 Redis / SessionRegistry 中将设备 1 会话标记为 KICKED，并把旧 JWT 写入黑名单
    │ 3. 调用 KeycloakSsoLogoutService 让 Keycloak 端的设备 1 会话立即失效
    │ 4. 发布 Kafka 事件：SESSION_INVALIDATED（loginSessionId = 设备1的 sid）
    │ 5. 注册设备 2 的新会话
    ▼
Kafka
    │
