...
### 3.12 通知表（sys_notification）

```sql
CREATE TABLE sys_notification (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL,               -- 目标用户ID（对应 keycloak_user_id / sys_user.id，应用层保证存在）
    type VARCHAR(50) NOT NULL,           -- 通知类型：FRIEND_REQUEST、SYSTEM_ALERT、GAME_INVITE 等
    title VARCHAR(200) NOT NULL,         -- 通知标题
    content TEXT NOT NULL,               -- 通知内容
    from_user_id VARCHAR(64),            -- 触发方用户ID（Keycloak userId，可选）
    ref_type VARCHAR(50),                 -- 关联业务类型（如 FRIEND_REQUEST）
    ref_id UUID,                         -- 关联业务ID
    payload JSONB,                       -- 透传数据（如 refId、requestMessage、roomId 等）
    actions TEXT[],                      -- 可选操作列表：['ACCEPT','REJECT'] 等
    status VARCHAR(20) NOT NULL DEFAULT 'UNREAD', -- UNREAD / READ / ARCHIVED / DELETED
    source_service VARCHAR(50),          -- 来源服务标识（如 system-service、game-service）
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    read_at TIMESTAMPTZ,
...
-- 表注释
COMMENT ON TABLE sys_notification IS '全局通知表（统一管理所有业务通知，支持离线推送和持久化）';
COMMENT ON COLUMN sys_notification.id IS '主键ID（UUID）';
COMMENT ON COLUMN sys_notification.user_id IS '目标用户ID（关联 sys_user.id，接收通知的用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN sys_notification.type IS '通知类型枚举：FRIEND_REQUEST（好友申请）、FRIEND_RESULT（好友申请结果）、SYSTEM_ALERT（系统提示）、GAME_INVITE（游戏邀请）、GAME_RESULT（游戏结果）等，必须非空';
COMMENT ON COLUMN sys_notification.title IS '通知标题（显示在通知列表，最大200字符，必须非空）';
COMMENT ON COLUMN sys_notification.content IS '通知内容（详细文案，TEXT类型，必须非空）';
COMMENT ON COLUMN sys_notification.from_user_id IS '触发方用户ID（Keycloak userId，如好友申请的发起人，可选）';
COMMENT ON COLUMN sys_notification.ref_type IS '关联业务类型（如：FRIEND_REQUEST，用于关联业务表，便于查询和去重）';
COMMENT ON COLUMN sys_notification.ref_id IS '关联业务ID（如：friend_request.id，用于关联业务记录，便于跳转和操作）';
COMMENT ON COLUMN sys_notification.payload IS '透传数据（JSONB，灵活存储业务相关数据，如：{"friendRequestId": "...", "requestMessage": "...", "requesterName": "...", "notificationId": "..."}）';
COMMENT ON COLUMN sys_notification.actions IS '可操作按钮列表（TEXT[]，如：["ACCEPT", "REJECT"] 表示显示"同意"和"拒绝"按钮，NULL 或空数组表示无需操作，仅展示）';
COMMENT ON COLUMN sys_notification.status IS '通知状态枚举：UNREAD（未读）、READ（已读）、ARCHIVED（已归档）、DELETED（已删除），默认 UNREAD，必须非空';
COMMENT ON COLUMN sys_notification.source_service IS '来源服务标识（如：system-service、game-service、chat-service，用于追踪通知来源）';
COMMENT ON COLUMN sys_notification.created_at IS '创建时间（TIMESTAMPTZ，自动设置，必须非空）';
COMMENT ON COLUMN sys_notification.read_at IS '已读时间（TIMESTAMPTZ，用户点击通知或标记已读时设置）';
COMMENT ON COLUMN sys_notification.archived_at IS '归档时间（TIMESTAMPTZ，用户归档通知时设置）';
COMMENT ON COLUMN sys_notification.deleted_at IS '删除时间（TIMESTAMPTZ，软删除，NULL 表示未删除）';
COMMENT ON COLUMN sys_notification.updated_at IS '更新时间（TIMESTAMPTZ，触发器自动更新）';
```

**字典值说明**：

1. **通知类型（type）**：
   - `FRIEND_REQUEST`：好友申请（接收方可见，带操作按钮）
   - `FRIEND_RESULT`：好友申请结果（申请人可见，无需操作）
   - `SYSTEM_ALERT`：系统提示（如：系统维护、活动通知等）
   - `GAME_INVITE`：游戏邀请（带操作按钮）
   - `GAME_RESULT`：游戏结果（对局结束通知）

2. **通知状态（status）**：
   - `UNREAD`：未读（显示在未读列表中，计入未读数量）
   - `READ`：已读（已查看，不计入未读数量）
   - `ARCHIVED`：已归档（用户主动归档，不显示在常规列表中）
   - `DELETED`：已删除（软删除，不显示）
...
4. **关联业务类型（ref_type）**：
   - `FRIEND_REQUEST`：关联好友申请表（friend_request）
   - `GAME_MATCH`：关联游戏对局表（game_match）
   - `GAME_ROOM`：关联游戏房间表（game_room）

5. **来源服务（source_service）**：
   - `system-service`：系统服务（用户、好友、通知等）
   - `game-service`：游戏服务（对局、房间等）
   - `chat-service`：聊天服务（消息通知等）

### 3.13 日志表分区模板（可选，规模化准备）
...
-- 表注释
COMMENT ON TABLE user_friend IS '好友关系表（双向关系，A加B为好友时，会创建两条记录：A→B 和 B→A）';
COMMENT ON COLUMN user_friend.id IS '主键ID（UUID）';
COMMENT ON COLUMN user_friend.user_id IS '用户ID（关联 sys_user.id，当前用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN user_friend.friend_id IS '好友用户ID（关联 sys_user.id，好友用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN user_friend.friend_nickname IS '好友备注昵称（用户为好友设置的备注名，最大50字符，可选）';
COMMENT ON COLUMN user_friend.friend_group IS '好友分组（如：游戏好友、现实好友、同事等，最大50字符，可选）';
COMMENT ON COLUMN user_friend.status IS '关系状态枚举：ACTIVE（正常好友关系）、BLOCKED（已拉黑，拉黑后不再接收对方消息和申请），默认 ACTIVE，必须非空';
COMMENT ON COLUMN user_friend.is_favorite IS '是否特别关心（true 表示特别关心，会在好友列表置顶显示），默认 false，必须非空';
COMMENT ON COLUMN user_friend.last_interaction_time IS '最后互动时间（TIMESTAMPTZ，如：最后聊天时间、最后对局时间等，用于排序和展示，可选）';
COMMENT ON COLUMN user_friend.created_at IS '创建时间（TIMESTAMPTZ，自动设置，必须非空）';
COMMENT ON COLUMN user_friend.updated_at IS '更新时间（TIMESTAMPTZ，触发器自动更新，必须非空）';
```

**字典值说明**：

1. **关系状态（status）**：
   - `ACTIVE`：正常好友关系（可以聊天、邀请游戏等）
   - `BLOCKED`：已拉黑（拉黑后不再接收对方消息和好友申请，但关系记录保留）

2. **特别关心（is_favorite）**：
   - `true`：特别关心（在好友列表中置顶显示）
   - `false`：普通好友（按最后互动时间排序）

3. **好友分组（friend_group）**：
   - 用户自定义分组名称（如：游戏好友、现实好友、同事等）
   - 可选字段，NULL 表示未分组

**设计说明**：
- **双向关系**：A 加 B 为好友时，会创建两条记录：`(user_id=A, friend_id=B)` 和 `(user_id=B, friend_id=A)`
- **唯一约束**：`UNIQUE(user_id, friend_id)` 保证同一用户不能重复添加同一好友
- **自检约束**：`CHECK (user_id != friend_id)` 防止用户添加自己为好友
...
-- 表注释
COMMENT ON TABLE friend_request IS '好友申请表（记录用户之间的好友申请，支持双向申请自动通过）';
COMMENT ON COLUMN friend_request.id IS '主键ID（UUID）';
COMMENT ON COLUMN friend_request.requester_id IS '申请人用户ID（关联 sys_user.id，发起好友申请的用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN friend_request.receiver_id IS '接收人用户ID（关联 sys_user.id，接收好友申请的用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN friend_request.request_message IS '申请留言（申请人填写的验证信息，最大200字符，可选，NULL 表示无留言）';
COMMENT ON COLUMN friend_request.status IS '申请状态枚举：PENDING（待处理，等待接收方同意/拒绝）、ACCEPTED（已接受，已建立好友关系）、REJECTED（已拒绝，接收方拒绝申请）、EXPIRED（已过期，超过有效期未处理），默认 PENDING，必须非空';
COMMENT ON COLUMN friend_request.handled_at IS '处理时间（TIMESTAMPTZ，接收方同意或拒绝时设置，可选，NULL 表示未处理）';
COMMENT ON COLUMN friend_request.created_at IS '创建时间（TIMESTAMPTZ，自动设置，必须非空）';
```

**字典值说明**：

1. **申请状态（status）**：
   - `PENDING`：待处理（等待接收方同意或拒绝，此时前端会显示操作按钮）
   - `ACCEPTED`：已接受（接收方同意申请，已建立双向好友关系，会创建两条 `user_friend` 记录）
   - `REJECTED`：已拒绝（接收方拒绝申请，不会建立好友关系）
   - `EXPIRED`：已过期（超过有效期未处理，由定时任务或应用层逻辑设置）

2. **申请留言（request_message）**：
   - 申请人填写的验证信息（如："我是XXX，想加你为好友"）
   - 最大200字符，可选字段，NULL 或空字符串表示无留言

**业务逻辑说明**：

1. **双向申请自动通过**：
   - 如果 A 向 B 发起申请（PENDING），同时 B 也向 A 发起申请（PENDING），系统会自动将两条申请都设置为 ACCEPTED，并创建双向好友关系

2. **唯一约束**：
   - `UNIQUE(requester_id, receiver_id) WHERE status = 'PENDING'` 保证同一对用户同一方向只能有一个待处理的申请
   - 防止重复申请，保证幂等性

3. **自检约束**：
   - `CHECK (requester_id != receiver_id)` 防止用户向自己发送申请

4. **状态流转**：
   - `PENDING` → `ACCEPTED`（接收方同意）
   - `PENDING` → `REJECTED`（接收方拒绝）
   - `PENDING` → `EXPIRED`（超时未处理，由定时任务设置）
...
COMMENT ON COLUMN chat_session.id IS '主键ID（UUID）';
COMMENT ON COLUMN chat_session.session_type IS '会话类型枚举：PRIVATE（私聊）、ROOM（房间聊天）、GROUP（群聊），必须非空';
COMMENT ON COLUMN chat_session.session_name IS '会话名称（群聊时使用，用于显示群聊名称，最大100字符，可选）';
COMMENT ON COLUMN chat_session.support_key IS '私聊会话唯一键（格式：min(user1,user2)||"|"||max(user1,user2)），用于去重，确保相同两个用户只能有一个PRIVATE会话，最大120字符，可选）';
COMMENT ON COLUMN chat_session.room_id IS '房间ID（房间聊天时使用，关联 game_room.id，应用层保证存在，可选，NULL 表示非房间会话）';
COMMENT ON COLUMN chat_session.created_by IS '创建人用户ID（群聊时使用，关联 sys_user.id，创建群聊的用户，应用层保证存在，可选，NULL 表示非群聊会话）';
COMMENT ON COLUMN chat_session.last_message_id IS '最后一条消息ID（关联 chat_message.id，用于快速获取最后一条消息，可选，NULL 表示暂无消息）';
COMMENT ON COLUMN chat_session.last_message_time IS '最后消息时间（TIMESTAMPTZ，用于会话列表排序，可选，NULL 表示暂无消息）';
COMMENT ON COLUMN chat_session.member_count IS '成员数量（必须非空，默认0，范围：>= 0，由触发器或应用层自动同步 chat_session_member 表）';
COMMENT ON COLUMN chat_session.created_at IS '创建时间（TIMESTAMPTZ，自动设置，必须非空）';
COMMENT ON COLUMN chat_session.updated_at IS '更新时间（TIMESTAMPTZ，触发器自动更新，必须非空）';
```
...
COMMENT ON COLUMN chat_session_member.id IS '主键ID（UUID）';
COMMENT ON COLUMN chat_session_member.session_id IS '会话ID（关联 chat_session.id，应用层保证存在，必须非空）';
COMMENT ON COLUMN chat_session_member.user_id IS '用户ID（关联 sys_user.id，会话成员的用户ID，应用层保证存在，必须非空）';
COMMENT ON COLUMN chat_session_member.member_role IS '成员角色枚举：MEMBER（成员）、ADMIN（管理员）、OWNER（群主），默认MEMBER，必须非空';
COMMENT ON COLUMN chat_session_member.nickname_in_session IS '在会话中的昵称（用户在该会话中设置的昵称，群聊时使用，最大50字符，可选，NULL 表示使用用户默认昵称）';
COMMENT ON COLUMN chat_session_member.joined_at IS '加入时间（TIMESTAMPTZ，用户加入会话的时间，自动设置，必须非空）';
COMMENT ON COLUMN chat_session_member.left_at IS '离开时间（TIMESTAMPTZ，用户离开会话的时间，可选，NULL 表示仍在会话中）';
COMMENT ON COLUMN chat_session_member.last_read_message_id IS '最后已读消息ID（关联 chat_message.id，用于显示未读消息数量，可选，NULL 表示未读任何消息）';
COMMENT ON COLUMN chat_session_member.last_read_time IS '最后已读时间（TIMESTAMPTZ，用户最后阅读消息的时间，用于判断消息是否已读，可选，NULL 表示未读）';
```
...
COMMENT ON COLUMN chat_message.id IS '主键ID（UUID）';
COMMENT ON COLUMN chat_message.session_id IS '会话ID（关联 chat_session.id，消息所属的会话，应用层保证存在，必须非空）';
COMMENT ON COLUMN chat_message.sender_id IS '发送者用户ID（关联 sys_user.id，发送消息的用户，应用层保证存在，必须非空）';
COMMENT ON COLUMN chat_message.client_op_id IS '客户端操作ID（前端生成UUID，用于幂等，防止重复发送，最大64字符，可选）';
COMMENT ON COLUMN chat_message.message_type IS '消息类型枚举：TEXT（文本消息）、IMAGE（图片消息）、FILE（文件消息）、SYSTEM（系统消息），默认TEXT，必须非空';
COMMENT ON COLUMN chat_message.content IS '消息内容（TEXT类型，消息的文本内容，必须非空，对于图片/文件类型，可存储描述或文件名）';
COMMENT ON COLUMN chat_message.content_tsv IS '全文检索向量（tsvector类型，由触发器自动生成，用于快速文本搜索，支持全文检索功能）';
COMMENT ON COLUMN chat_message.extra_data IS '扩展数据（JSONB，灵活存储消息的扩展信息，如：{"image_url": "...", "file_name": "...", "file_size": 1024, "width": 800, "height": 600}，默认空对象）';
COMMENT ON COLUMN chat_message.reply_to_message_id IS '回复的消息ID（关联 chat_message.id，表示该消息是对哪条消息的回复，可选，NULL 表示非回复消息）';
COMMENT ON COLUMN chat_message.is_recalled IS '是否已撤回（BOOLEAN，true 表示消息已被发送者撤回，false 表示未撤回，默认false，必须非空）';
COMMENT ON COLUMN chat_message.recalled_at IS '撤回时间（TIMESTAMPTZ，消息被撤回的时间，可选，NULL 表示未撤回）';
COMMENT ON COLUMN chat_message.created_at IS '创建时间（TIMESTAMPTZ，消息发送时间，自动设置，必须非空）';
```
