...
3. **Token 存储**（session-common Redis，database: 0）：
   - WebSocket Token：`ws:token:{sessionId}` (String)
   - TTL：1 小时
...
public class ChatWsController {
    ...
    public void sendPrivate(@Payload SendPrivateMessage cmd, SimpMessageHeaderAccessor sha) {
        ...
        // 确保 ThreadLocal 中有 Token（供 Feign 调用使用）
...
**使用 `JwtTokenHolder`（ThreadLocal）**：

1. **Token 存储**：`web-common` 库提供的 `JwtTokenHolder`，使用 ThreadLocal 存储 Token

2. **Token 设置时机**：
   - **WebSocket CONNECT**：`WebSocketAuthChannelInterceptor` 在 CONNECT 时设置 Token
   - **REST API 请求**：Gateway 的 TokenRelay 过滤器自动设置 Token

3. **Feign 拦截器**：`FeignAuthAutoConfiguration`（来自 `web-common`）自动配置 Feign 拦截器
...
#### 5.3.3 WebSocket 内的 Feign 调用

**关键问题**：WebSocket 消息处理是异步的，ThreadLocal 可能丢失
...
1. **CONNECT 时保存 Token**：
   ...
   if (StompCommand.CONNECT.equals(accessor.getCommand())) {
       // 保存 Token 到 Redis（以 sessionId 为 key）
...
3. **可靠性**：Redis 存储支持多实例部署，内存降级支持单实例
...
3. **Token 传递**：使用 `WebSocketTokenStore`（Redis + 内存降级）存储 Token，供 Feign 调用使用
4. **Feign 调用**：通过 `JwtTokenHolder`（ThreadLocal）和 Feign 拦截器自动传递 Token
