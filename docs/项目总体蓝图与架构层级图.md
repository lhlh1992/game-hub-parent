...
- `games/gomoku/`：五子棋（规则判定、AI、对局状态、WS 控制器、TurnClockCoordinator）
  ...
  - `application/`：应用编排层（倒计时协调器）
  - `infrastructure/redis/`：Redis 实现（仓储实现、键规范）
- `clock/`：通用倒计时引擎（业务无关的 `CountdownScheduler` 与实现）
- `application/`：应用层（跨游戏通用服务，如用户目录服务）
- `infrastructure/redis/`：全局 Redis 基础设施（连接配置、工具类）
- `platform/ws/`：WebSocket 配置与广播工具
- `platform/config/`：平台配置（Spring Security、OAuth2 Resource Server）
...
- 建议：统一使用 JSON 序列化（跨版本/跨语言友好），显式管理 TTL 与清理
...
- **恢复扫描**：当前实现使用 `redis.keys("countdown:*")`（第134行），存在阻塞风险，建议改用 SCAN 游标遍历
- **holder 锁**：TTL 10秒，回调完成后清理/幂等，避免多节点重复执行超时回调

**AI 执行并发控制**：
- **本地防抖**：使用 `ConcurrentHashMap<String, ScheduledFuture<?>> pendingAi`，同一房间只保留一个待执行的 AI 任务
- **分布式互斥**：当前实现缺少房间级分布式锁，多节点部署可能重复执行，建议使用 Redis SETNX（`ai:lock:{roomId}` + TTL）
...
| `RedisGameStateRepository` | 棋局状态 Redis 实现（支持 CAS）。 |
...
### 6.1 game-service 演进路线

1. **接入网关与鉴权（P0）**：
   - Gateway+Auth → game-service 内部一律基于 userId 授权
   - `seatKey` 仅辅助恢复，不作为主要鉴权凭证
   - **当前状态**：✅ 已实现 Gateway JWT 验证，game-service 从 JWT 提取 userId，WebSocket 通过 URL 参数传递 token 并验证

2. **修复与加固（P0）**：
   - 倒计时 `stop` 清理（已完成）：删除状态键与 holder 锁，避免重启误恢复
   - 倒计时恢复改用 SCAN：当前使用 `redis.keys()`（第134行），存在阻塞风险，建议改用 SCAN 游标遍历
   - turnKey TTL/清理：回合锚点设置合理 TTL，终局/重开/stop 时显式删除
   - holder 锁 TTL 可配置：当前硬编码 10 秒，建议外置化配置
   - AI 执行分布式互斥：当前缺少房间级分布式锁，建议使用 Redis SETNX（`ai:lock:{roomId}` + TTL）
   - AI CAS 的 expectedTurn 修正：当前硬编码 `Board.WHITE`（第429行），应使用 `now.current()` 作为 expectedTurn
...
1. **链路验收（P0）**：
   - **当前状态**：✅ Keycloak→Gateway→game-service→Session 事件链路已打通
   - **待完成**：补齐部署/配置文档与自动化脚本，确保新人一键拉起即可复现

2. **聊天室 MVP（P0）**：
   - **当前状态**：❌ 前端有 GlobalChat 组件，但未接入后端
   - **待完成**：抽象消息模型，落地大厅聊天 & 房间聊天（含消息持久化、禁言、WS 广播）；未来频道/私聊都在此基础上扩展

3. **后台治理（P1）**：
   - **当前状态**：✅ 会话监控 API 已实现（`/internal/sessions`），✅ 踢线功能已实现
   - **待完成**：补齐菜单/权限配置界面、审计日志，为运营提供最小闭环

4. **服务拆分（P1）**：
   - **当前状态**：Lobby 功能内聚在 game-service，匹配功能未实现
   - **待完成**：Lobby/Engine 分离，并沉淀 `game-engine-template`（房间、匹配、结算接口约定）以便快速孵化新小游戏

5. **数据持久化（P1）**：
   - **当前状态**：✅ system-service 用户数据已持久化到 PostgreSQL，❌ 对局记录未持久化
   - **待完成**：对局与聊天历史写入 PostgreSQL/MongoDB，开放战绩/排行榜/回放，为玩家档案与成长体系提供数据基座

6. **可观测性与部署（P1）**：
   - **当前状态**：✅ Docker Compose 已配置，❌ K8s 未配置，❌ Prometheus/Grafana 未接入
   - **待完成**：Docker Compose → K8s，多环境配置治理，Prometheus/Grafana + OpenTelemetry，形成统一监控告警

7. **扩展服务（P2）**：
   - **当前状态**：❌ 未实现
   - **待完成**：Chat 进阶（频道、私聊、战队）、Matchmaking、Rating、Analytics 及多游戏运营工具，支持活动、赛事与 BI
...
- **聊天室 MVP**：❌ 未实现
  - **当前状态**：前端有 GlobalChat 组件，但未接入后端
  - **待完成**：实现公共大厅 + 房间聊天，消息先存 Redis（队列 + 近期历史），带禁言/屏蔽接口
- **玩家中心雏形**：❌ 未实现
  - **当前状态**：ProfilePage 为占位页面
  - **待完成**：`/me` 展示战绩/最近房间（先写 Mock/Redis 统计），为后续成长体系做入口

### Phase C：平台化演进（按需排期）
- **Lobby/Engine 拆分**：❌ 未实现
  - **当前状态**：Lobby 功能内聚在 game-service，匹配功能未实现
  - **待完成**：抽离匹配/房间服务，产出 `game-engine-template`，为第二款小游戏埋下钩子
- **战绩 & 数据**：❌ 未实现
  - **当前状态**：战绩只存在 Redis，未持久化
  - **待完成**：补 PostgreSQL schema，落地对局记录、排行榜 API
- **可观测性/部署**：❌ 未实现
  - **当前状态**：只有 Docker Compose，未配置 K8s，未接入 Prometheus/Grafana
  - **待完成**：加入 Prometheus/Grafana，完善 CI/CD（GitHub Actions + Docker Registry），再考虑更多游戏、频道聊天、运营工具
